...
<h1 class="text-center mb-3 text-white">FBS Deputy Proctor</h1>
        ...
        @foreach($pendingUnionEvents as $event)
            <div class="card mb-3 p-3 d-flex flex-row justify-content-between align-items-center">
                ...
                <div>
                    ...
                    <form action="{{ route('fbsdpUnion.accept', $event->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
                        <button class="btn btn-success btn-sm me-2">Accept</button>
                    </form>
                    <form action="{{ route('fbsdpUnion.reject', $event->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
    ...
    @else
        @foreach($pendingBatchEvents as $event)
            <div class="card mb-3 p-3 d-flex flex-row justify-content-between align-items-center">
                ...
                <div>
                    {!! renderImageButton($event->image_path) !!}
                    <form action="{{ route('fbsdpBatch.accept', $event->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
                        <button class="btn btn-success btn-sm me-2">Accept</button>
                    </form>
                    <form action="{{ route('fbsdpBatch.reject', $event->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
                        <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $event->id }}">
                            Reject

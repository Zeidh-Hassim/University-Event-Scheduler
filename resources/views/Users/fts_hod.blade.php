...
<h1 class="text-center mb-3 text-white">FTS HOD</h1>
        ...
        @foreach($pendingSocietyEvents as $event)
            <div class="card mb-3 p-3 d-flex flex-row justify-content-between align-items-center">
                ...
                <div>
                    ...
                    <form action="{{ route('ftshodSociety.accept', $event->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
                        <button class="btn btn-success btn-sm me-2">Accept</button>
                    </form>
                    <form action="{{ route('ftshodSociety.reject', $event->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')

...
<h1 class="text-center mb-3 text-white">Assistant Registrar</h1>
...
<div class="container mt-5">
    ...
    @else
        ...
        <div class="card mb-3 p-3 d-flex flex-row justify-content-between align-items-center">
            ...
            <div>
                <form action="{{ route('ar.accept', $event->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('PATCH')
                    <button class="btn btn-success btn-sm me-2">Accept</button>
                </form>

                <form action="{{ route('ar.reject', $event->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('PATCH')
